...
| 5 | Structured request interpreter | Versioned bounded schema; original text retained; invalid/time-limited model output rejected; explicit commands work without a model. | Foundation in development: strict shared proposal parser/prompt and CPU evaluation adapter; runtime inference, clarification commands and EEF integration remain planned |
